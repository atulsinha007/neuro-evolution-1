	...
	while c1 < len1 or c2 < len2:
		...
		if (f1 == 1 and f1 == f2 and i.innov_num == j.innov_num):
			...
			if i.source.node_num not in nodeDict.keys():
				nodeObj1 = gene.Node(i.source.node_num, i.source.nature)
			...
			if i.destination.node_num not in nodeDict.keys():
				nodeObj2 = gene.Node(i.destination.node_num, i.destination.nature)
			...
			conObj = gene.Conn(i.innov_num, (nodeObj1, nodeObj2), wt, stat)  # conn object
			...
			while c1 < length:
				...
				if i.destination.node_num not in nodeDict.keys():
					...
					nodeObj2 = nodeDict[i.destination.node_num]
				c1 += 1
				connObj = gene.Conn(i.innov_num, (nodeObj1, nodeObj2), i.weight, i.status)
...
def crossoverTest(parentx, parenty, gen_no, inputdim=8, outputdim=1):
	...
	for i in range(len(output_nodes)):
		if i < point_of_crossover:
			...
			wt = parent2.bias_conn_arr[i].weight
		new_bias_conn = gene.BiasConn(output_nodes[i], wt)
		# gene.Conn(-1, (gene.Node(-1, -1), output_nodes[i]), wt, True)
		child.bias_conn_arr.append(new_bias_conn)
